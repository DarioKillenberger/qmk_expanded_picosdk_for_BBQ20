...
void __attribute__((weak)) runtime_init_bootrom_reset(void) {
...
#include "pico/bootrom.h"
void __attribute__((weak)) runtime_init_per_core_bootrom_reset(void) {
    // todo can we tell if we came in thru the bootrom where this is not necessary (this is necessary for debugger)
    rom_bootrom_state_reset_fn state_reset = rom_func_lookup(ROM_FUNC_BOOTROM_STATE_RESET);
...
void __attribute__((weak)) runtime_init_early_resets(void) {
...
void __attribute__((weak)) runtime_init_usb_power_down(void) {
...
// #include "hardware/structs/m33.h" // Problematic include for RP2040 (M0+)
// ----------------------------------------------------
// 00200 PICO_RUNTIME_INIT_PER_CORE_ENABLE_COPROCESSORS
// ----------------------------------------------------
void __attribute__((weak)) runtime_init_per_core_enable_coprocessors(void) {
    // VFP copro (float)
    // uint32_t cpacr = M33_CPACR_CP10_BITS; // M33 specific
//#if HAS_DOUBLE_COPROCESSOR
    // cpacr |= M33_CPACR_CP4_BITS; // M33 specific
//#endif
//#if PICO_USE_GPIO_COPROCESSOR
    // cpacr |= M33_CPACR_CP0_BITS; // M33 specific
//#endif
    // arm_cpu_hw->cpacr |= cpacr; // This would need to be SCB->CPACR for M0+ and use M0+ specific bits if enabling FPU (which M0+ doesn't have)
//#if HAS_DOUBLE_COPROCESSOR
    // asm volatile ("mrc p4,#0,r0,c0,c0,#1" : : : "r0"); // clear engaged flag via RCMP
//#endif
    // For RP2040 (M0+), this function as written seems incorrect.
    // The M0+ CPACR is for FPU (CP10/CP11), which RP2040 doesn't have.
    // Other coprocessors like interpolator/divider are not enabled this way.
}
#endif
...
#include "hardware/resets.h"
void __attribute__((weak)) runtime_init_post_clock_resets(void) {
...
void __attribute__((weak)) runtime_init_rp2040_gpio_ie_disable(void) {
...
void __attribute__((weak)) runtime_init_spin_locks_reset(void) {
    spin_locks_reset();
}
#endif
