...
void clocks_init(void) {
    ...
    // This will use your 20MHz crystal if XOSC_MHZ is correctly set to 20
    ...
    // Enable the xosc
    // This initializes your 20MHz crystal based on XOSC_MHZ
    xosc_init();
    ...
    // Configure PLLs
    // Assumes XOSC_MHZ is set to 20 (for your 20MHz crystal)
    // PLL SYS: 20MHz / 1 = 20MHz * 60 (fbdiv) = 1200MHz (VCO)
    //          1200MHz / 5 / 5 = 48MHz (clk_sys)
    // PLL USB: 20MHz / 1 = 20MHz * 60 (fbdiv) = 1200MHz (VCO)
    //          1200MHz / 5 / 5 = 48MHz (clk_usb)

    // Parameters for pll_init: pll_init(pll_instance, refdiv, vco_target_freq, post_div1, post_div2)
    // fbdiv is calculated internally by pll_init based on xosc_get_hz() and vco_target_freq
    pll_init(pll_sys, 1, 1200 * MHZ, 5, 5); // Target 48MHz for clk_sys
    pll_init(pll_usb, 1, 1200 * MHZ, 5, 5); // Target 48MHz for clk_usb

    // Configure clocks
    // CLK_REF = XOSC (20MHz) / 1 = 20MHz
    clock_configure(clk_ref,
                    CLOCKS_CLK_REF_CTRL_SRC_VALUE_XOSC_CLKSRC,
                    0, // No aux mux
                    20 * MHZ, // Source frequency is 20MHz from XOSC
                    20 * MHZ); // Target frequency is 20MHz

    // CLK SYS = PLL SYS (48MHz) / 1 = 48MHz
    clock_configure(clk_sys,
                    CLOCKS_CLK_SYS_CTRL_SRC_VALUE_CLKSRC_CLK_SYS_AUX,
                    CLOCKS_CLK_SYS_CTRL_AUXSRC_VALUE_CLKSRC_PLL_SYS,
                    48 * MHZ,  // Source frequency from PLL SYS is 48MHz
                    48 * MHZ);  // Target frequency is 48MHz

    // CLK USB = PLL USB (48MHz) / 1 = 48MHz
    clock_configure(clk_usb,
                    0, // No GLMUX
                    CLOCKS_CLK_USB_CTRL_AUXSRC_VALUE_CLKSRC_PLL_USB,
                    48 * MHZ, // Source frequency from PLL USB is 48MHz
                    48 * MHZ); // Target frequency is 48MHz
    ...
    clock_configure(clk_rtc,
                    ...
                    48 * MHZ, // Source frequency from PLL USB is 48MHz
                    46875);   // Target frequency is 46.875kHz

    // CLK PERI = clk_sys.
    // Source frequency for clk_peri is now the new clk_sys frequency (48MHz)
    clock_configure(clk_peri,
                    0,
                    CLOCKS_CLK_PERI_CTRL_AUXSRC_VALUE_CLK_SYS,
                    48 * MHZ,  // Source frequency from clk_sys is 48MHz
                    48 * MHZ);  // Target frequency is 48MHz
}
